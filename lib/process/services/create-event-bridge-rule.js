...
const createEventBridgeRule = async (tenant, company) => {
  ...
  if (company.siteMode === 'both' || company.siteMode === 'sale') {
    ...
    const targetsResult = await eventBridge.send(new PutTargetsCommand({
    ...
    if (targetsResult.FailedEntryCount > 0) {
      throw new Error(`Falha ao configurar targets da regra ${ruleNameRebuildSalePage}: ${JSON.stringify(targetsResult.FailedEntries)}`);
    }
  ...
  if (company.siteMode === 'both' || company.siteMode === 'rent') {
    ...
    if (targetsResult.FailedEntryCount > 0) {
      throw new Error(`Falha ao configurar targets da regra ${ruleNameRebuildRentPage}: ${JSON.stringify(targetsResult.FailedEntries)}`);
    }
