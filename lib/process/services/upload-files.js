...
const buildCacheControll = (filePath, key) => {
  if (!['.html', '.css', '.js'].includes(path.extname(filePath).toLowerCase())) {
    return;
  }
  return key.split('/')[1] === 'administrativo' ? 'no-cache' : 'public, max-age=28800, must-revalidate'
}

const uploadFile = async (filePath, key) => {
  const fileStream = fs.createReadStream(filePath);
  const CacheControl = buildCacheControll(filePath, key);
  const uploadParams = {
