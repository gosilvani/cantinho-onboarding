    ...
    2: {
      validate: () => {
        ...
        const districts = Array.from(
          document.querySelectorAll('#districts input[type="checkbox"]:checked')
        ).map(cb => cb.value);
        ...
        formData.criteria.districts =
          selectDistrictMode.value === 'select'
            ? districts
            : citiesAndDistricts
              .find(c => c.name === selectCity.value)
              .districts.filter(d => !districts.includes(d));
