...
const buildComponentProperty = (property) => {
  ...
  titleValue.textContent = `R$ ${property.billings.salePrice.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
  ...
  itemInfoRooms.textContent = `${property.details.rooms} quartos`;
  listDetails.appendChild(itemInfoRooms);
  const itemGarage = document.createElement('li');
  itemGarage.textContent = `${property.details.garage_spaces} gararage${property.details.garage_spaces > 1 ? 's' : ''}`;
...
const openPopup = (propertyIdentifier, isDirect) => {
  ...
  span.insertAdjacentHTML('afterbegin', iconDetail);
  divInfos.appendChild(span);

  const divInfosDetails = document.createElement('div');
  const itemRooms = document.createElement('span');
  itemRooms.textContent = `${property.details.rooms} quarto${property.details.rooms > 1 ? 's' : ''}`;
  const itemBathrooms = document.createElement('span');
  itemBathrooms.textContent = `${property.details.bathrooms} banheiro${property.details.bathrooms > 1 ? 's' : ''}`;
  const itemGarage = document.createElement('span');
  itemGarage.textContent = `${property.details.garage_spaces} garage${property.details.garage_spaces > 1 ? 'ns' : 'm'}`;
  ...
  strongValueSalePrice.textContent = `R$ ${property.billings.salePrice.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
  ...
  const buildResponsible = (propertyResponsible, contentProperty) => {
    const { owner_name, company, contact, site, logo } = propertyResponsible;
    const defineContentResponsible = (company && logo && site) ? 'company' : 'owner';
    const strategyFunction = {
      ...
      owner: () => {
        ...
        spanNameOwner.textContent = owner_name;
  ...
  if (!isDirect) {
    ...
    divActions.appendChild(buttonClose);
  } else {
    const buttonClose = document.createElement('button');
    const spanButton = document.createElement('span');
...
const processRegistDemand = async (context) => {
  const name = capitalizeLetter(document.getElementById(`name_${context}`).value, 'firstLetter');
  ...
  dataDemand.contact.whatsapp = whatsapp;
