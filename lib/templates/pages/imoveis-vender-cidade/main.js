...
const handleCityChange = (event) => {
  ...
  const propertyType = propertyTypeSelected
    ...
    .replace(/[\u0300-\u036f]/g, '')
    .toLocaleLowerCase();
  const targetPath = `/imoveis/venda/${citySelected}/${propertyType}/index.html`;
...
const processRegistDemand = async (context) => {
  ...
  const value = parseFloat(document.getElementById(`price_range_${context}`).value);
...
const applyFilters = () => {
  ...
  const priceRange = parseFloat(document.getElementById('price_range').value) || 999999999;
  const rooms = parseInt(document.getElementById('rooms').value) || 0;
  const garageSpaces = parseInt(document.getElementById('garage_spaces').value) || 0;
  ...
  propertiesResult = properties.filter(property =>
    ...
    priceRange >= property.billings.salePrice &&
...
const cleanFilters = () => {
  filtered = false;
  ['price_range', 'rooms', 'garage_spaces'].forEach(id => { document.getElementById(id).selectedIndex = 0; });
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (propertyTypeSelected) {
    ...
    districtsOptions.forEach(district => {
      ...
      checkbox.checked = true;
      label.append(checkbox, document.createTextNode(district));
  ...
  document.querySelector('.search_result').addEventListener('click', (event) => {
    const ad = event.target.closest('article.ad');
    if (!ad) return;

    if (event.target.closest('.btn_prev')) {
      navigateCarousel(ad.dataset.id, 'prev', 'article');
      return;
    }
    if (event.target.closest('.btn_next')) {
      navigateCarousel(ad.dataset.id, 'next', 'article');
      return;
    }
    openPopup(ad.dataset.id);
  });

  const params = new URLSearchParams(window.location.search);
  const propertyId = params.get('id');
  if (propertyId) openPopup(propertyId, true);
