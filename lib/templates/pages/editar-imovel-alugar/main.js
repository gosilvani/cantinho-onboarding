...
const buildData = () => {
  ...
  if (!city || !district || !streetInput || !type || !accept_pets_input) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (commercialTypes.includes(propertyData.details.type)) {
    selectAcceptPets.value = '0';
  } else {
  ...
  document.getElementById('garage_spaces').value = String(propertyData.details.garage_spaces);
  ...
  if (isMarketplace) {
    ...
    inputContact.value = propertyData.responsible.contact;
